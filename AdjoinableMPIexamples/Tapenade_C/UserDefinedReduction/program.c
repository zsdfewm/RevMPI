...
//TODO: make the Fortran equivalent
...
void head(OurType *x, OurType *y, int len, MPI_Datatype mpi_OurType, MPI_Comm comm) {
  MPI_Op op_oursum ;
  int i ;
  MPI_Op_create(oursum, 1, &op_oursum) ;
  MPI_Reduce(x, y, len, mpi_OurType, op_oursum, 0, comm);
  for (i=0 ; i<len ; ++i) {
...
int main(int argc, char** argv) {
  ...
  MPI_Type_create_struct(3, blockLengths, blockOffsets, blockTypes, &mpi_OurType) ;
