...
int head(double* in, double *out) { 
  ...
  MPI_Scatterv(sBuf, counts, displs,MPI_DOUBLE, rBuf, rBufSize, MPI_DOUBLE, root, MPI_COMM_WORLD);
  if (world_rank != root)
    for (i=0;i<vSize;++i) { 
      rBuf[i] = 3*(rBuf[i]);
    }
  ...
  MPI_Gatherv(rBuf, rBufSize, MPI_DOUBLE, sBuf, counts, displs, MPI_DOUBLE, root, MPI_COMM_WORLD);
  if (world_rank == root) {
    for(r=0;r<gSize;++r) { 
      for (i=0;i<vSize;++i) { 
	y+=sBuf[r*vSize+i];
