...
void head(adouble* x, adouble *y) { 
  ...
  if (world_rank == 0) {
    *x=*x*2;
    AMPI_Irecv (y, 1, TYPE_MACRO, 1, 0, AMPI_FROM_SEND, MPI_COMM_WORLD, &r);
    AMPI_Send(x, 1, TYPE_MACRO, 1, 0, AMPI_TO_IRECV_WAIT,       MPI_COMM_WORLD);
    AMPI_Wait(&r,MPI_STATUS_IGNORE);
    *y=*y*3;
  } else if (world_rank == 1) {
    adouble local;
    AMPI_Irecv (&local, 1, TYPE_MACRO, 0, 0, AMPI_FROM_SEND, MPI_COMM_WORLD, &r);
    AMPI_Wait(&r,MPI_STATUS_IGNORE);
    local=sin(local);
    AMPI_Send (&local, 1, TYPE_MACRO, 0, 0, AMPI_TO_IRECV_WAIT, MPI_COMM_WORLD);
  } 
}
