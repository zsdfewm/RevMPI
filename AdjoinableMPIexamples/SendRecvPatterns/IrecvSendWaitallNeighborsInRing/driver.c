...
int main(int argc, char** argv) {
  ...
  MPI_Request r[2]; 
  sBuf = 3.0*world_rank;
  MPI_Irecv(&rBuf[0], 1, MPI_DOUBLE, left_rank,  0, MPI_COMM_WORLD, &r[0]);
  MPI_Irecv(&rBuf[1], 1, MPI_DOUBLE, right_rank, 0, MPI_COMM_WORLD, &r[1]);
  MPI_Send(&sBuf, 1, MPI_DOUBLE, left_rank,  0, MPI_COMM_WORLD);
  MPI_Send(&sBuf, 1, MPI_DOUBLE, right_rank, 0, MPI_COMM_WORLD);
