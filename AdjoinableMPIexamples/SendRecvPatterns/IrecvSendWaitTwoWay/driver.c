...
int main(int argc, char** argv) {
  ...
  if (world_rank == 0) {
    sBuf = 3.14;
    MPI_Irecv(&rBuf, 1, MPI_DOUBLE, 1, 0, MPI_COMM_WORLD, &r);
    MPI_Send (&sBuf, 1, MPI_DOUBLE, 1, 0, MPI_COMM_WORLD);
    printf(__FILE__ ": process 0 received number %f from process 1\n", rBuf);
    MPI_Wait(&r,MPI_STATUS_IGNORE);
    printf(__FILE__ ": process 0 is done waiting for Irecv\n");
  } else if (world_rank == 1) {
    sBuf = 6.28;
    MPI_Irecv(&rBuf, 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, &r);
    MPI_Send (&sBuf, 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
