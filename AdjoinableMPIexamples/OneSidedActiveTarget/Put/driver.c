...
int head(double* x, double *y) { 
  ...
  if (world_rank == 0) {
    ...
    MPI_Win_create(y, sizeof(double), sizeof(double), MPI_INFO_NULL, MPI_COMM_WORLD, &yWin);
    MPI_Win_fence(0, yWin);
    /* y's value is being put here by rank 1 */
    MPI_Win_fence(0, yWin);
    MPI_Win_free(&yWin);
  } else if (world_rank == 1) {
    MPI_Win emptyWin,localWin;
    double local=0.0;
    ...
    MPI_Win_create(NULL, 0, 1, MPI_INFO_NULL, MPI_COMM_WORLD, &emptyWin);
    MPI_Win_fence(0, emptyWin);
    MPI_Put(&local,1,MPI_DOUBLE,0,0,1,MPI_DOUBLE,emptyWin);
    MPI_Win_fence(0, emptyWin);
    MPI_Win_free(&emptyWin);
  } 
}
